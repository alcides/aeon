...
# The metric/oracle helpers and the reference solution carry the `__internal__`
# prefix so they stay out of the synthesizer's grammar (they are excluded from
# `concrete_vars`); the search must build the `Csg` itself, not call the scorer
# or return the answer. This replaces the old `let jaccard = unit in ...` shadow.
...
def __internal__shape_solution : Csg = __REF__;
"""
