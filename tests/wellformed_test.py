...
def test_wf1():
    ...
    assert not wellformed(empty, TypeVar(Name("a")))
    assert wellformed(empty.with_typevar("a", BaseKind()), TypeVar(Name("a")))
    assert wellformed(empty.with_typevar("a", StarKind()), TypeVar(Name("a")))
...
def test_poly():
    assert wellformed(empty, TypePolymorphism("a", BaseKind(), TypeVar(Name("a"))))
    assert wellformed(empty, TypePolymorphism("a", StarKind(), TypeVar(Name("a"))))
    assert not wellformed(
        empty,
        TypePolymorphism("a", StarKind(), TypeVar(Name("a"))),
        BaseKind(),
    )
