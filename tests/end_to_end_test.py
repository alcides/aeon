...
def test_anf():
    source = r"""let f : (x:Int) -> (y:Int) -> Int = (\x -> (\y -> x)) in
           ...
           r"""
    check_compile_expr(source, top, 1)
...
def test_anf_typed_smaller():
    ...
    source = "let x = 3 in x"
    check_compile_expr(source, parse_type("{x:Int | x == 3}"), 3)


def test_annotation():
    source = r""" (1 : Int) """
    check_compile_expr(source, parse_type("Int"), 1)
...
def test_annotation_anf3():
    source = r"""3 % 2"""
    check_compile_expr(source, parse_type("{x:Int | x == 1}"), 1)
