...
Both reuse ``GESynthesizer`` -- still used by ``hc`` / ``1p1`` -- so the rest of
the suite only smoke-constructs them (``synthesizer_seed_test``). These confirm
the factory wires each strategy and that each actually synthesizes a well-typed
term for a basic-typed hole.
"""
