...
from aeon.types import ExistentialType, TypeException, Type, TypingContext, BasicType, AbstractionType, TypeAbstraction, \
    IntersectionType, UnionType, TypeApplication, RefinedType, top, bottom, find_basic_types, shape, star, Type
...
class VariableState(Type):
    ...
    def __str__(self):
        return "var({} l={} u={})".format(self.name, self.lower_bounds,
                                          self.upper_bounds)
...
def constrain(ctx: TypingContext, t1: Type, t2: Type,
              constraints: Dict[str, VariableState]):
    ...
    if isinstance(t1, BasicType) and isinstance(t2, BasicType) and t1 == t2:
        ...
        constrain(ctx.with_var(t2.arg_name, t2.arg_type), k2, t2.return_type,
                  constraints)
        ...
        constrain(ctx.with_var(t1.arg_name, k1), t1.return_type, k2,
                  constraints)
        ...
        constrain(ctx, t1.argument, t2.argument,
                  constraints)  # TODO: variance?

    elif isinstance(t1, ExistentialType):
        constrain(ctx, t1.right, t2,
                  constraints)  #.with_var(t1.left_name, t1.left)

    elif isinstance(t2, ExistentialType):
        constrain(
            ctx, t1, t2.right, constraints
        )  # .with_var(t2.left_name, t2.left) # TODO: is this correct?

    elif isinstance(t1, IntersectionType):
        if isinstance(t1.right, RefinedType):
            talt = RefinedType(t1.right.name, shape(t2),
                               smt_not(t1.right.cond))
...
def collapse(t: BasicType, explored: List[str], polarity: bool,
             constraints: Dict[str, VariableState]):
    r: Type
    if polarity:
        r = top
        if t.name not in explored:
            for high in constraints[t.name].upper_bounds:
                if isinstance(high,
                              BasicType) and high.name in constraints.keys():
                    high = collapse(high, explored + [t.name], not polarity,
                                    constraints)
        ...
        if t.name not in explored:
            for low in constraints[t.name].lower_bounds:
                if isinstance(low,
                              BasicType) and low.name in constraints.keys():
                    low = collapse(low, explored + [t.name], not polarity,
                                   constraints)
                r = UnionType(r, low)
        else:
            r = BasicType(t.name)
...
def unification(ctx: TypingContext, t1: Type, t2: Type) -> Type:
    type_variables = []
    type_kinds = {}
    original_t1 = t1
    ...
    variables_to_abstract = [(t, type_kinds[t])
                             for t in variables_to_track.keys() if t in bts]
    r = reduce(lambda y, a: TypeAbstraction(a[0], a[1], y),
               variables_to_abstract, r)
    return r


def unificationEq(ctx: TypingContext, t1: Type, t2: Type) -> Type:
    type_variables1 = []
    type_kinds1 = {}
    type_variables2 = []
    original_t1 = t1
    ...
    bts = find_basic_types(r)
    variables_to_abstract = [(t, variables_to_track[t].kind)
                             for t in variables_to_track.keys() if t in bts]
    r = reduce(lambda y, a: TypeAbstraction(a[0], a[1], y),
               variables_to_abstract, r)
    return r
