...
@aefunction('empty_llist[T]() -> {l:LList[T] | l.size == 0} = native;', lambda x : empty_list(x))
def empty_list(ignore):
    return List(None, None)


@aefunction('append_llist[T](x:T, l:LList[T]) -> {l2:LList[T] | l2.size == l.size + 1} = native;', lambda x: lambda l: append(x, l))
def append(x, l):
    result = List(None, None)
...
@aefunction('extend_llist[T](l1:LList[T], l2:LList[T]) -> {l3:LList[T] | l3.size == l1.size + l2.size} = native;', lambda l1: lambda l2: extend(l1, l2))
def extend(l1, l2):
    result = List(None, None)
    current = result
...
@aefunction('insert_llist[T](x:T, l:LList[T], {i:Integer | i <= l.size}) -> {l2:LList[T] | l2.size == l.size + 1} = native;', lambda x: lambda l: lambda i: insert(x, l, i))
def insert(x, l, i):
    if i == 0:
...
@aefunction('remove_llist[T](x:T, l:LList[T]) -> {l2:LList[T] | l2.size <= l.size} = native;', lambda x: lambda l: remove(x, l))
def remove(x, l):
    if not l:
        return None
    if not l.head:
...
@aefunction('contains_llist[T](x:T, l:LList[T]) -> Boolean = native;', lambda x: lambda l: contains(x, l))
def contains(x, l):
    result = l.head == x
...
@aefunction('index_llist[T](x:T, l:LList[T]) -> {i:Integer | -1 <= i && i < l.size} = native;', lambda x: lambda l: index(x, l, 0))
def index(x, l, i):
    if x == l.head:
        return i
    else:
...
@aefunction('count_llist[T](x:T, l:LList[T]) -> Integer = native;', lambda x: lambda l: count(x, l))
def count(x, l):
    result = (int) (x == l.head)
    if l.next:
...
@aefunction('exists_llist[T](f:(x:T -> Boolean), l:LList[T]) -> Boolean = native;', lambda f: lambda l: exists(f, l))
def exists(f, l):
    if not l.head:
        return False
    result = f(l.head)
...
@aefunction('forall_llist[T](f:(x:T -> Boolean), l:LList[T]) -> Boolean = native;', lambda f: lambda l: forall(f, l))
def forall(f, l):
    if not l.head:
        return False
    result = f(l.head)
...
# Check if any element of the list respects a predicate 
@aefunction('filter_llist[T](f:(x:T -> Boolean), l:LList[T]) -> LList[T] = native;', lambda f: lambda l: filter(f, l))
def filter(f, l):
    if not l.head:
        return List(None, None)
    if f(l.head):
...
# Map each element
@aefunction('map_llist[T](f:(x:T -> T), l:LList[T]) -> {l2:LList[T] | l.size == l2.size} = native;', lambda f: lambda l: map(f, l))
def map(f, l):
    if not l.head:
        return List(None, None)
...
# Size of the list 
@aefunction('length_llist[T](l:LList[T]) -> {i:Integer | i == l.size} = native;', lambda l: length(l))
def length(l):
