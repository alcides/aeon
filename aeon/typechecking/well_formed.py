...
def wellformed(ctx: TypingContext, t: Type, k: Kind = StarKind()) -> bool:
    ...
    wf_base = (isinstance(t, RefinedType) and isinstance(t.type, BaseType)
    ...
    wf_var_base = (isinstance(t, RefinedType) and isinstance(t.type, TypeVar)
                   and k == BaseKind()
                   and (t.type.name, BaseKind()) in ctx.typevars()
                   and type_infer_liquid(ctx.with_var(t.name, t.type),
                                         t.refinement) == t_bool)

    wf_var = isinstance(t, TypeVar) and (t.name
                                         in [v[0] for v in ctx.typevars()])
    wf_var2 = (isinstance(t, RefinedType) and isinstance(t.type, TypeVar)
