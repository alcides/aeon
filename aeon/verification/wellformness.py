...
def wellformed_constraint_liquid(ctx: LiquidTypeCheckingContext,
                                 c: Constraint) -> None:
    match c:
        case LiquidConstraint(expr):
            assert check_liquid(
                ctx, expr,
                t_bool), f"Expected boolean expression, got {expr}."
        ...
        case UninterpretedFunctionDeclaration(name, ft, seq):
            nctx = LiquidTypeCheckingContext(
                ctx.known_types, ctx.variables,
                ctx.functions | {name: lower_abstraction_type(ft)})
            assert isinstance(
                ft,
                AbstractionType), f"{ft} is expected to be an AbstractionType."
            wellformed_constraint_liquid(nctx, seq)
        case TypeVarDeclaration(name, seq):
            nctx = LiquidTypeCheckingContext(
                ctx.known_types + [BaseType(name)], ctx.variables,
                ctx.functions)
        ...
        case Implication(name, base, pred, seq):
            ...
            nctx = LiquidTypeCheckingContext(ctx.known_types,
                                             ctx.variables | {name: base},
                                             ctx.functions)
            assert isinstance(base,
                              BaseType), f"Expected BaseType, got {base}."
...
def canonicalize_type(t: Type) -> Type:
    match t:
        ...
        case AbstractionType(name, at, rt):
            return AbstractionType(name, canonicalize_type(at),
                                   canonicalize_type(rt))
        case _:
            assert False, f"No canonicalization for type {t} ({type(t)})"


def canonicalize_constraint(c: Constraint,
                            stack: list[Name] = None) -> Constraint:
    if stack is None:
        stack = []
    ...
    match c:
        ...
        case Conjunction(c1, c2):
            return Conjunction(canonicalize_constraint(c1, stack),
                               canonicalize_constraint(c2, stack))
        case UninterpretedFunctionDeclaration(name, ft, seq):
            new_name = Name(name, fresh_counter.fresh())
            if new_name != name:
            ...
            assert isinstance(
                nty, AbstractionType), f"Expected AbstractionType, got {nty}."
        ...
        case TypeVarDeclaration(name, seq):
            ...
            return TypeVarDeclaration(
                name, canonicalize_constraint(seq, stack + [name]))
        case Implication(name, base, pred, seq):
            new_name = Name(name, fresh_counter.fresh())
            if new_name != name:
            ...
            assert (isinstance(nty, BaseType) or isinstance(nty, Top) or
                    isinstance(nty, TypeVar)), f"Expected BaseType, got {nty}."
