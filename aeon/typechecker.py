    ...
    def handle_aliases(self, t, recursive=False):
        ...
        if recursive and type(t) == Type:
            ...
            if t.lambda_parameters:
                t.lambda_parameters = list(
                    map(self.handle_aliases, t.lambda_parameters))
            if t.type_arguments:
                t.type_arguments = list(
                    map(self.handle_aliases, t.type_arguments))
            ...
            if possible_generic_type.binders:
                for v in possible_generic_type.binders:
                    for ct in self.types:
                        ft_concrete = possible_generic_type.copy_replacing_freevar(
                            v, ct)
                        if self.is_subtype(t, ft_concrete, do_aliases=False):
                            b = self.type_aliases[
                                possible_generic_type].copy_replacing_freevar(
                                    v, ct)
                            t.type = b
                            t.consolidate()
                            return t
    ...
    def unify(self, invocation_type, return_type):
        ...
        for binder in invocation_type.binders:
            if binder == invocation_type.type:
                return invocation_type.copy_replacing_freevar(
                    binder, invocation_type.type)
        ...
        if ft.binders:
            for v in ft.binders:
                for ct in self.types + [v]:
                    ...
                    a, ft_concrete_r = self.check_function_arguments(
                        args, ft_concrete)
                    if a:
            ...
            valid = all([
                self.is_subtype(a, b)
                for a, b in zip(args, ft.lambda_parameters)
            ])
            return (valid, ft)
    ...
    def define_fun(self, name, type_, n):
        ...
        if name not in self.funs:
            ...
            if len(self.funs[name]) == 2:
                ...
                self.funs[name][0] = (original_new_name, self.funs[name][0][1],
                                      self.funs[name][0][2])
...
class TypeChecker(object):
    ...
    def t_native(self, n, expects=None):
        ...
        argtypes = [
            self.typecontext.resolve_type(x.nodes[1]) for x in n.nodes[1]
        ]
        ft = Type(lambda_parameters=argtypes,
                  ...
                  effects=n.nodes[5])
        ft.set_conditions(n.nodes[4], [n.nodes[2].nodes[0]],
                          [x.nodes[0] for x in n.nodes[1]])
        ft.set_effects(n.nodes[5], [n.nodes[2].nodes[0]],
                       [x.nodes[0] for x in n.nodes[1]])
    ...
    def t_decl(self, n, expects=None):
        ...
        if not self.is_subtype(real_type, n.type):
            self.type_error(
                "Function {} expected {} and body returns {}".format(
                    name, n.type, real_type),
                expected=n.type,
                given=real_type)
        if self.refined:
            if not self.is_subtype(real_type, n.type, under=(ft, argtypes)):
                self.type_error(
                    "Function {} failed post-condition {} when returning {}".
                    format(name, ft, real_type),
                    given=real_type,
                    expected=ft)
    ...
    def t_invocation(self, n, expects=None):
        ...
        actual_argument_types = [
            self.typecontext.resolve_type(c.type) for c in n.nodes[1]
        ]

        if len(actual_argument_types) != len(t.lambda_parameters):
            self.type_error("Wrong number of arguments for {}({})".format(
                name, ",".join(map(str, actual_argument_types))),
                            expected=t.lambda_parameters,
                            given=actual_argument_types)

        valid, concrete_type = self.check_function_arguments(
            actual_argument_types, t_name)
        if valid:
            concrete_type.type = self.typecontext.resolve_type(
                concrete_type.type)  # Return type
            n.type = concrete_type.type
        else:
            self.type_error(
                "Wrong argument types for {} | Expected {} -- Got {}".format(
                    name, str(t_name),
                    str(list(map(str, actual_argument_types)))),
                given=Type(basic=expects or "?",
                           lambda_parameters=actual_argument_types),
                expected=t_name)

        if self.refined:
            if name == "J.iif":
                ok, ref_name = zed.refine_function_invocation(
                    name,
                    concrete_type,
                    actual_argument_types,
                    cond=n.nodes[1][0])
                ...
                ok, ref_name = zed.refine_function_invocation(
                    name, concrete_type, actual_argument_types)
            if not ok:
                self.type_error(
                    "Refinement checking failed for invocation {} in {}".
                    format(name, pp(n)),
    ...
    def t_lambda(self, n, expects=None):
        ...
        for arg in n.nodes[0]:
            if self.context.find(arg[0]) != None:
                self.type_error(
                    "Argument {} of lambda {} is already defined as variable".
                    format(arg[0], str(n)))
            self.context.set(arg[0], arg[1])
        self.typecheck(n.nodes[1], expects=expects)
        self.context.pop_frame()

        n.type = Type(lambda_parameters=args, basic=n.nodes[1].type)
        ...
        if len(n.nodes) > 2 and n.nodes[2]:
            ...
            if not n.coerced:
                ...
                if not self.is_subtype(n.nodes[1].type, n.type):
                    self.type_error(
                        "Variable {} is not of type {}, but rather {}".format(
                            n.nodes[0], n.type, n.nodes[1].type),
                        expected=n.type,
    ...
    def t_op(self, n, expects=None):
        if n.nodet in ['&&', '||', '==>']:
            ...
            for c in n.nodes:
                if c.type != t_b:
                    self.type_error(
                        "{} expected boolean arguments in {}. Got {}.".format(
                            n.nodet, c, c.type),
                        expected=t_b,
                        given=c.type)
        elif n.nodet in ['!']:
            ...
            if n.nodet == "%" and n.type != t_i:
                self.type_error(
                    "Operator {} can only be applied to integer, not {} and {}"
                    .format(n.nodet, n.nodes[0].type, n.nodes[1].type))
            if self.refined:
                n.type.refined = zed.combine(n.type, n.nodet, n.nodes)
                n.type.context = zed.copy_assertions()
        else:
    ...
    def t_literal(self, n, expects=None):
        ...
        if self.refined:
            ...
            n.type.context = zed.copy_assertions()
            n.type.conditions = [
                Node('==', Node('atom', 'self'), Node('literal', n.nodes[0]))
            ]
        else:
            pass
