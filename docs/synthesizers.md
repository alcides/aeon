...
Enumerates candidates with a **lazy min-heap over synthesis levels** (default ``synquid_search: "size_merge"`` in hole metadata): primary key is ``term_size``, then shallower ``level``, so a small term at a deeper level can precede a large shallow one (``aeon.synthesis.modules.synquid.search.iter_candidates_size_then_level``). Set ``synquid_search: "iterative_deepening"`` to restore strict “exhaust level *L* before *L+1*” behaviour. **Within** each level, candidates are still sorted by ``term_size``. Optional caps: ``synquid_max_level`` (default 128) and ``synquid_seed_levels`` (default 2, min 1) tune the heap merge; iterative deepening also stops after ``synquid_max_level``. Set ``synquid_typecheck_candidate_first`` to run ``check_type`` on the candidate in the **hole** typing context before substituting into the full program (can skip work on ill-typed candidates; keep off unless you know the hole context matches full-program checking). It prunes function applications when the goal return type is incompatible with the callee’s result (spine decomposition), and **prioritises `if` guards** from **qualifier atoms** `Q` plus bounded **pairwise ``&&``** combinations (same `Q` as Horn; see Jhala & Vazou, [Refinement Types: A Tutorial](https://arxiv.org/abs/2010.07763)). Arrow types **preserve refined domains** on the spine so argument holes inherit those refinements. ``aeon.synthesis.modules.synquid.modular.application_subgoal_types`` exposes per-argument goals for tooling. If the hole has **no** fitness ``goals``, the synthesizer **returns the first fully type-correct candidate** and stops. Remaining gaps vs the paper include full round-trip liquid checking on partial terms, MUSFIX-class abduction, and backtracking on failed partial checks.
