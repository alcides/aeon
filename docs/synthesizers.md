...
**Search.** Default ``synquid_search: "size_merge"`` uses a **lazy min-heap** over synthesis levels (``term_size``, then shallower ``level``) via ``aeon.synthesis.modules.synquid.search.iter_candidates_size_then_level``. ``synquid_search: "iterative_deepening"`` restores strict “exhaust level *L* before *L+1*”. **Within** each level, candidates are sorted by ``term_size``. Tunables: ``synquid_max_level``, ``synquid_seed_levels``; optional ``synquid_typecheck_candidate_first`` runs ``check_type`` on the candidate in the **hole** context before full-program validation.

**Enumeration.** Level 0 supports **Bool**, **Int**, **Float**, **String** (small fixed literal set), and variables. For an **arrow** goal at level ≥ 1 it emits **λ-abstractions** (``Abstraction``) by synthesising the body under an extended context, in addition to applications from the typing context. ``if`` branches try **ternary**, then **binary**, then **unary** relational guards built from **Q**, then plain boolean enumeration. Function applications are **pruned** when the callee’s result type does not match the goal (spine decomposition); refined argument types are preserved on the spine. ``aeon.synthesis.modules.synquid.modular`` exposes ``application_subgoal_types`` for subgoals and ``check_hole_term`` as a thin ``check_type`` wrapper for whole candidates; ``aeon.synthesis.modules.synquid.build`` re-exports the main engine, search, and modular entry points.

**Fitness.** If the hole has **no** ``goals`` metadata, the first fully valid candidate is returned.

**Not in scope (vs the PLDI Synquid paper or ideal liquid tooling):** **MUSFIX-style** weakest-guard abduction (only bounded ``&&``-products over **Q**), **no** separate modular liquid calculus on arbitrary **partial** syntax fragments (only ``check_type`` on whole hole terms and full-program checking after substitution), **no** core-level ``match`` / ``fix`` (those live in sugar + elaboration), and **Horn** lifting still **skips** polymorphic and refinement-polymorphic value binders (see comments in ``aeon/typechecking/entailment.py``). For further background on **Q** and refinement Horn encodings, see Jhala & Vazou, [Refinement Types: A Tutorial](https://arxiv.org/abs/2010.07763).
