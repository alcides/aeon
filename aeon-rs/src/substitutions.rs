...
fn make_abstraction_type(
    ...
    let inst = Py::new(
        py,
        (AbstractionType { var_name, var_type, type_, loc }, Type),
    )?;
    Ok(inst.into_any())
}
...
pub fn type_substitution(
    ...
    if let Ok(rt) = bound.downcast::<RefinedType>() {
        ...
        if let Ok(inner_rt) = new_inner_bound.downcast::<RefinedType>() {
            ...
            let qualifiers = py.import_bound("aeon.core.liquid_ops")?;
            let mk_liquid_and = qualifiers.getattr("mk_liquid_and")?;
            ...
            let merged = mk_liquid_and.call1((rt.refinement.clone_ref(py), substituted))?;
...
pub fn refined_to_unrefined_type(py: Python<'_>, ty: PyObject) -> PyResult<PyObject> {
    ...
    if let Ok(at) = bound.downcast::<AbstractionType>() {
        ...
        return make_abstraction_type(py, at.var_name.clone_ref(py), nv, nt, at.loc.clone_ref(py));
    }
    Ok(ty)
}
