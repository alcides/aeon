...
impl SAbstractionType {
    ...
    fn __str__(&self, py: Python<'_>) -> PyResult<String> {
        // Match Python's pretty-printer: prefix the multiplicity unless it is MOmega.
        let mult_obj = self.multiplicity.bind(py);
        let momega = crate::types::default_multiplicity(py);
        let prefix = if mult_obj.is(momega.bind(py)) {
            String::new()
        } else {
            format!("{} ", mult_obj.str()?)
        };
...
impl SLet {
    ...
    fn py_new(
        ...
        let multiplicity = multiplicity.unwrap_or_else(|| crate::types::default_multiplicity(py));
        (
            SLet { var_name, var_value, body, loc: resolve_loc(py, loc), multiplicity },
            STerm,
        )
    }
...
pub struct Definition {
    ...
    #[pyo3(get, set)]
    pub arg_multiplicities: PyObject,
    #[pyo3(get, set)]
    pub loc: PyObject,
}
...
impl Definition {
    #[new]
    #[pyo3(signature = (name, foralls, args, r#type, body, decorators = None, rforalls = None, decreasing_by = None, loc = None, arg_multiplicities = None))]
    ...
    fn py_new(
        ...
        loc: Option<PyObject>,
        arg_multiplicities: Option<PyObject>,
    ) -> PyResult<(Self, Node)> {
        ...
        let mk = || PyList::empty_bound(py).unbind();
        let arg_multiplicities = arg_multiplicities
            .unwrap_or_else(|| PyTuple::empty_bound(py).into_any().unbind());
        Ok((
            Definition {
                name,
                foralls,
    ...
    fn multiplicity_of(&self, py: Python<'_>, i: usize) -> PyResult<PyObject> {
        let am = self.arg_multiplicities.bind(py);
        let len: usize = am.len()?;
        if i < len {
            Ok(am.get_item(i)?.unbind())
        } else {
            Ok(crate::types::default_multiplicity(py))
        }
    }
